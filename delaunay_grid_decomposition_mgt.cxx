    ...
    , group_intervals(NULL)
{
    ...
    *this->kernel_boundry = *this->expanded_boundry = boundry;
    ...
    expanding_scale[0] = expanding_scale[1] = expanding_scale[2] = expanding_scale[3] = 2;
    num_neighbors_on_boundry[0] = num_neighbors_on_boundry[1] = num_neighbors_on_boundry[2] = num_neighbors_on_boundry[3] = 0;
}
...
void Search_tree_node::decompose_by_processing_units_number(double *workloads, double *child_points_coord[4], int *child_points_idx[2], int child_num_points[2],
                                   Boundry child_boundry[2], vector<int> child_proc_units_id[2], int mode, int *c_intervals[2], int c_num_intervals[2])
{
...
int Delaunay_grid_decomposition::insert_virtual_points(double *coord_values[2], Boundry *boundry, int num_points)
{
    ...
    maxX = minX = coord_values[PDLN_LON][0];
    maxY = minY = coord_values[PDLN_LAT][0];
    for(int i = 0; i < num_points; i++)
    {
    ...
    double r_minx, r_maxx, r_miny, r_maxy;
    v_minx = minX-deltaMax*PDLN_INSERT_EXPAND_RATIO;
    v_maxx = maxX+deltaMax*PDLN_INSERT_EXPAND_RATIO;
    v_miny = minY-deltaMax*PDLN_INSERT_EXPAND_RATIO;
    v_maxy = maxY+deltaMax*PDLN_INSERT_EXPAND_RATIO;

    r_minx = is_cyclic && v_minx < 0   ?   1 : v_minx;
    r_maxx = is_cyclic && v_maxx > 360 ? 359 : v_maxx;
    r_miny = v_miny < -90 ? -89 : v_miny;
    r_maxy = v_maxy > 90  ?  89 : v_maxy;
...
void Delaunay_grid_decomposition::initialze_workload()
{
    ...
    int regionID = 0;
    Processing_unit** p_units = processing_info->get_processing_units();
...
void Delaunay_grid_decomposition::decompose_common_node_recursively(Search_tree_node *node, bool lazy_mode)
{
    ...
    int *children_intervals[2];
    int children_num_intervals[2];
    ...
    node->decompose_by_processing_units_number(workloads, child_points_coord, child_points_index, 
                                               child_num_points, child_boundry, child_proc_id, 
                                               PDLN_DECOMPOSE_COMMON_MODE, children_intervals, 
                                               children_num_intervals);
    ...
    node->children[0]->set_groups(children_intervals[0], children_num_intervals[0]);
    node->children[2]->set_groups(children_intervals[1], children_num_intervals[1]);
    for(int i = 0; i < 4; i++)
                                                                     ...
                                                                     unsigned *remote_checksums, vector<MPI_Request*> *waiting_list, int iter)
{
    ...
    unsigned checksum, boundry_type;
